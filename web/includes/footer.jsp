<%@page contentType="text/html" pageEncoding="UTF-8"%>
    </div> <!-- End container -->
    
    <!-- Footer Section -->
    <footer class="mt-auto border-top bg-white" style="color: var(--color-foreground);">
        <!-- Main Footer Links -->
        <div class="container py-5">
            <div class="row g-4">
                <!-- Col 1: Brand & About -->
                <div class="col-lg-4 col-md-6">
                    <div class="navbar-brand-wrapper mb-3">
                        <span class="brand-box">161</span>
                        <span class="brand-box">GARMENT</span>
                    </div>
                    <p class="text-secondary small mb-4 pe-lg-4" style="line-height: 1.7;">
                        Thương hiệu thời trang phong cách tối giản chuẩn Nhật Bản. Chúng tôi chú trọng sự tinh tế, độ bền và cảm giác thoải mái nhất cho người mặc mỗi ngày.
                    </p>
                    <div class="d-flex gap-2">
                        <a href="#" class="btn btn-outline-dark btn-sm rounded-0" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
                            <i class="fa-brands fa-facebook-f"></i>
                        </a>
                        <a href="#" class="btn btn-outline-dark btn-sm rounded-0" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
                            <i class="fa-brands fa-instagram"></i>
                        </a>
                        <a href="#" class="btn btn-outline-dark btn-sm rounded-0" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
                            <i class="fa-brands fa-tiktok"></i>
                        </a>
                        <a href="#" class="btn btn-outline-dark btn-sm rounded-0" style="width: 36px; height: 36px; display: inline-flex; align-items: center; justify-content: center;">
                            <i class="fa-brands fa-youtube"></i>
                        </a>
                    </div>
                </div>

                <!-- Col 2: About Us -->
                <div class="col-lg-2 col-md-3 col-6">
                    <h6 class="fw-bold text-uppercase mb-3 tracking-wide" style="font-size: 0.85rem; letter-spacing: 0.5px;">VỀ 161 GARMENT</h6>
                    <ul class="list-unstyled small d-flex flex-column gap-2 mb-0">
                        <li><a href="home" class="text-secondary text-decoration-none hover-link">Trang chủ</a></li>
                        <li><a href="product" class="text-secondary text-decoration-none hover-link">Tất cả sản phẩm</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Câu chuyện thương hiệu</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Hệ thống cửa hàng</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Tuyển dụng</a></li>
                    </ul>
                </div>

                <!-- Col 3: Customer Support -->
                <div class="col-lg-2 col-md-3 col-6">
                    <h6 class="fw-bold text-uppercase mb-3 tracking-wide" style="font-size: 0.85rem; letter-spacing: 0.5px;">TRỢ GIÚP & HỖ TRỢ</h6>
                    <ul class="list-unstyled small d-flex flex-column gap-2 mb-0">
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Hướng dẫn chọn size</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Chính sách đổi trả 30 ngày</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Chính sách giao hàng</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Tra cứu đơn hàng</a></li>
                        <li><a href="#" class="text-secondary text-decoration-none hover-link">Câu hỏi thường gặp (FAQ)</a></li>
                    </ul>
                </div>

                <!-- Col 4: Newsletter & Contact -->
                <div class="col-lg-4 col-md-12">
                    <h6 class="fw-bold text-uppercase mb-3 tracking-wide" style="font-size: 0.85rem; letter-spacing: 0.5px;">NHẬN BẢN TIN KHUYẾN MÃI</h6>
                    <p class="text-secondary small mb-3">Đăng ký để nhận thông tin bộ sưu tập mới nhất và voucher giảm giá 10%.</p>
                    
                    <form class="d-flex mb-3" onsubmit="event.preventDefault(); alert('Cảm ơn bạn đã đăng ký nhận bản tin của 161 Garment!');">
                        <input type="email" class="form-control rounded-0 border-dark small" placeholder="Nhập email của bạn..." required>
                        <button type="submit" class="btn btn-danger rounded-0 fw-bold px-3 text-uppercase small" style="background-color: var(--color-primary); letter-spacing: 0.5px;">
                            ĐĂNG KÝ
                        </button>
                    </form>
                    
                    <div class="small text-secondary">
                        <div><i class="fa-solid fa-phone me-2 text-danger"></i> Hotline: <strong class="text-dark">1900 161 161</strong> (8:30 - 22:00)</div>
                        <div class="mt-1"><i class="fa-solid fa-envelope me-2 text-danger"></i> Email: <strong class="text-dark">cskh@161garment.vn</strong></div>
                    </div>
                </div>
            </div>
        </div>

        <!-- Sub Footer -->
        <div class="border-top py-3 bg-light">
            <div class="container d-flex flex-column flex-md-row justify-content-between align-items-center gap-2 small text-muted">
                <div>
                    &copy; 2026 <strong>161 GARMENT</strong>. All Rights Reserved. PRJ301 Project.
                </div>
                <div class="d-flex align-items-center gap-3 fs-5 text-secondary">
                    <i class="fa-brands fa-cc-visa" title="Visa"></i>
                    <i class="fa-brands fa-cc-mastercard" title="Mastercard"></i>
                    <i class="fa-solid fa-money-bill-wave" title="Thanh toán khi nhận hàng (COD)"></i>
                    <i class="fa-solid fa-shield-halved" title="Bảo mật thanh toán"></i>
                </div>
            </div>
        </div>
    </footer>

    <style>
        .hover-link:hover {
            color: var(--color-primary) !important;
            text-decoration: underline !important;
        }
    </style>

    <!-- Bootstrap 5 JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
